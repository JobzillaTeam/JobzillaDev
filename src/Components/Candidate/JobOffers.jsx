...
const JobOffers = () => {
  ...
  useEffect(() => {
    setIsLoading(true)
    ApiServicesOrgCandidate.getJobOffersForCandidate().then(response => {
        ...
        <section className="content_section">
          <div className="row">
            <div className="col-md-12 py-4">
              <h5 className="job-heading">Job Offers in Pipeline</h5>
              <p className="job-invite mb-0">You have {cloneResourceJobs && cloneResourceJobs.length ? cloneResourceJobs.length : 0} job offers</p>
            </div>
          </div>
                    ...
                    return (
                      <div className="jobListItem" key={jobDetails.jobId}>
                        <section className={`row mx-0 px-4 ${resourceIndex !== 0 && 'pt-4'} pb-4`}>
                          ...
                          <div className="col-12 px-0 text-right marB10"><Link to={{ pathname: `/candidate/jobDetails/jobOffers/${jobDetails.jobId}` }}>view details <img src="/images/icons/view_details_arrow.svg" className="detail-arrow" /></Link></div>
                        </section>
                      </div>
                    )
                  }) : null
...
export default React.memo(JobOffers)